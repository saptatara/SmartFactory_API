...
def customer_dashboard(request):
    ...
    recent_data = SensorData.objects.filter(device__in=devices).order_by("-created_at")[:5]
...
def get_sensor_data(request, device_id):
    ...
    sensor_data = defaultdict(list)
    for r in SensorData.objects.filter(device=device).order_by("-created_at"):
        sensor_data[r.sensor_config.sensor_label].append({
...
def iot_data_list(request):
    data = IoTData.objects.filter(device__customer__user=request.user).order_by("-created_at")[:100]
